...
router.get('/getComments',loginCheck,async (ctx,next) => {
	const { content } = ctx.request.query
	ctx.body = await getComments({content})
})

router.post('/deleteComments',loginCheck,async (ctx,next) => {
...
router.post('/updateComment',loginCheck,async (ctx,next) => {
